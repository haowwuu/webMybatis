/*
 * File Name: XMLServiceBuilder.java
 ...
 * 
 * @author Wuwuhao
 * @version DSPlite V0.2.0, 2016-9-13
 * @see
...
public class XMLServiceBuilder extends BaseBuilder {
	...
	public XMLServiceBuilder(Reader reader) {
		this(reader, null);
	}

	public XMLServiceBuilder(Reader reader, String environment) {
		this(reader, environment, null);
	}

	public XMLServiceBuilder(Reader reader, String environment, Properties props) {
		this(new XPathParser(reader, true, props, new XMLMapperEntityResolver()), environment, props);
	}

	public XMLServiceBuilder(InputStream inputStream) {
		this(inputStream, null);
	}

	public XMLServiceBuilder(InputStream inputStream, String environment) {
		this(inputStream, environment, null);
	}

	public XMLServiceBuilder(InputStream inputStream, String environment, Properties props) {
		this(new XPathParser(inputStream, true, props, new XMLMapperEntityResolver()), environment, props);
	}

	private XMLServiceBuilder(XPathParser parser, String environment, Properties props) {
	...
	public Configuration parse() {
		if (parsed) {
			throw new BuilderException("Each XMLServiceBuilder can only be used once.");
		}
		this.parsed = true;
		parseService(parser.evalNode("/service"));
	...
	private void parseService(XNode root) {
		try {
			this.configuration = dataCenterElement(root.evalNode("dataCenter"));
			this.builderAssistant = new MapperBuilderAssistant(configuration, null);
	...
	private Configuration dataCenterElement(XNode context) throws Exception {
		if (null != context) {
			...
			if (resource != null && url == null && mapperClass == null) {
				...
				XMLDataCenterConfigBuilder mapperParser = new XMLDataCenterConfigBuilder(
						inputStream, configuration, resource, this.environment);
				return mapperParser.parse();
			} else if (resource == null && url != null && mapperClass == null) {
				ErrorContext.instance().resource(url);
				InputStream inputStream = Resources.getUrlAsStream(url);
				XMLDataCenterConfigBuilder mapperParser = new XMLDataCenterConfigBuilder(
						inputStream, configuration, url, this.environment);
				return mapperParser.parse();
			} else if (resource == null && url == null && mapperClass != null) {
	...
	private void mapperElement(XNode context) {
		try {
			...
			if(namespace.indexOf(".")<0){
				namespace = "com.cetiti.data."+namespace;  //soap service need a full qualified name space
			}
	...
	private Discriminator processDiscriminatorElement(XNode context,
			Class<?> resultType, List<ResultMapping> resultMappings) throws Exception {
		...
		if (namespace != null) {
			...
			if (boundType != null) {
				if (!configuration.hasMapper(boundType)) {
					...
					// flag to prevent loading again this resource from the mapper
					// interface look at MapperAnnotationBuilder#loadXmlResource
